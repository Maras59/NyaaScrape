...
class MainCLI(cmd.Cmd):
    ...
    def do_addtorrent(self, line):
        ...
        date = ''  # TODO: Cleanup, add default date
        ...
        maxfilesize = None
        maxfilesize = input(
            f'Enter max file size [{conf["FULL_BATCH_THRESHOLD"]}]: '
        ).strip()
        if maxfilesize:
            try:
                ...
                print(
                    f'Max file size must be a valid number, defaulting to {conf["FULL_BATCH_THRESHOLD"]}'
                )
        else:
        ...
        if confirm in ['y', 'yes']:
            ...
            try:
                with open(
                    conf['SHOW_CSV_PATH'], mode='a', newline='', encoding='utf-8'
                ) as file:
